...
if not os.path.exists("./dataCKD/dict"):
    os.makedirs("./dataCKD/dict")
if not os.path.exists("./dataCKD/csv"):
    os.makedirs("./dataCKD/csv")
...
class Generator():
    ...
    def generate_adm(self):
        data=pd.read_csv(f"./dataCKD/cohort/{self.cohort_output}.csv.gz", compression='gzip', header=0, index_col=None)
    ...
    def generate_cond(self):
        cond=pd.read_csv("./dataCKD/features/preproc_diag_icu.csv.gz", compression='gzip', header=0, index_col=None)
    ...
    def generate_proc(self):
        proc=pd.read_csv("./dataCKD/features/preproc_proc_icu.csv.gz", compression='gzip', header=0, index_col=None)
    ...
    def generate_out(self):
        out=pd.read_csv("./dataCKD/features/preproc_out_icu.csv.gz", compression='gzip', header=0, index_col=None)
    ...
    def generate_chart(self):
        chunksize = 5000000
        final=pd.DataFrame()
        for chart in tqdm(pd.read_csv("./dataCKD/features/preproc_chart_icu.csv.gz", compression='gzip', header=0, index_col=None,chunksize=chunksize)):
    ...
    def generate_meds(self):
        meds=pd.read_csv("./dataCKD/features/preproc_med_icu.csv.gz", compression='gzip', header=0, index_col=None)
...
#         else:
    ...
    def create_chartDict(self,chart,los):
        ...
        ######SAVE DICTIONARIES##############
        with open("./dataCKD/dict/metaDic", 'rb') as fp:
            metaDic=pickle.load(fp)
        
        with open("./dataCKD/dict/dataChartDic", 'wb') as fp:
            pickle.dump(dataDic, fp)

      
        with open("./dataCKD/dict/chartVocab", 'wb') as fp:
            pickle.dump(list(chart['itemid'].unique()), fp)
        self.chart_vocab = chart['itemid'].nunique()
        metaDic['Chart']=self.chart_per_adm
        
            
        with open("./dataCKD/dict/metaDic", 'wb') as fp:
...
#         print("# Unique insurance",self.data.insurance.nunique())
        ...
        for hid in tqdm(self.hids):
            ...
            if not os.path.exists("./dataCKD/csv/"+str(hid)):
                os.makedirs("./dataCKD/csv/"+str(hid))
            demo_csv.to_csv('./dataCKD/csv/'+str(hid)+'/demo.csv',index=False)
    ...
    #                 print(amount.columns)
            ...
            dyn_csv.to_csv('./dataCKD/csv/'+str(hid)+'/dynamic.csv',index=False)
        ...
        if(self.feat_cond):
            with open("./dataCKD/dict/condVocab", 'wb') as fp:
        ...
        if(self.feat_proc):    
            with open("./dataCKD/dict/procVocab", 'wb') as fp:
