...
if not os.path.exists("./data2/behrt"):
    os.makedirs("./data2/behrt")
...
class train_behrt():
    def __init__(self,src, age, sex, ethni, ins, target_data):
        ...
        labels = pd.read_csv("./data2/behrt/behrt_labels.csv", header=None)
        preds = pd.read_csv("./data2/behrt/behrt_preds.csv", header=None)
        ...
        def eval(_valload, saving, device):
            ...
            behrt.eval()
            if saving:
                with open("./data2/behrt/behrt_preds.csv", 'w') as f:
                    f.write('')
                with open("./data2/behrt/behrt_labels.csv", 'w') as f:
                    f.write('')
            ...
            for step, batch in enumerate(_valload):
                ...
                if saving:
                    with open("./data2/behrt/behrt_preds.csv", 'a') as f:
                        pd.DataFrame(logits.detach().cpu().numpy()).to_csv(f, header=False)
                    with open("./data2/behrt/behrt_labels.csv", 'a') as f:
