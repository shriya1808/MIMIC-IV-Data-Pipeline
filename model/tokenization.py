...
class BEHRT_models():
    ...
    def tokenize(self):
        ...
        demo_list = []
        cond_list = []
        labels =  pd.read_csv('./dataCOPD/csv/'+'labels.csv')
        first = True
        #labels = labels.iloc[:1, :]
        print("STARTING READING FILES.")
        for hadm in tqdm.tqdm(labels.itertuples(), total = labels.shape[0]):
            labs = pd.read_csv('./dataCOPD/csv/' + str(hadm[1]) + '/dynamic.csv')
            demo = pd.read_csv('./dataCOPD/csv/' + str(hadm[1]) + '/demo.csv')
            cond = pd.read_csv('./dataCOPD/csv/' + str(hadm[1]) + '/static.csv')
            if first:
        ...
        with open('./dataCOPD/dict/ethVocab', 'rb') as fp:
            ethVocab_l = pickle.load(fp)
            for i in range(len(ethVocab_l)):
                ethVocab[ethVocab_l[i]] = i

        with open('./dataCOPD/dict/insVocab', 'rb') as fp:
