...
class ML_models():
    ...
    def create_kfolds(self):
        labels=pd.read_csv('./dataCOPD/csv/labels.csv', header=0)
    ...
    def ml_train(self):
        ...
        for i in range(self.k_fold):
            ...
            concat_cols=[]
            if(self.concat):
                dyn=pd.read_csv('./dataCOPD/csv/'+str(train_hids[0])+'/dynamic.csv',header=[0,1])
    ...
    def getXY(self,ids,labels,concat_cols):
        ...
        for sample in ids:
            ...
            #print(sample)
            dyn=pd.read_csv('./dataCOPD/csv/'+str(sample)+'/dynamic.csv',header=[0,1])
...
#             print(dyn_df.shape)
#             print(dyn_df.head())
            stat=pd.read_csv('./dataCOPD/csv/'+str(sample)+'/static.csv',header=[0,1])
            stat=stat['COND']
#             print(stat.shape)
#             print(stat.head())
            demo=pd.read_csv('./dataCOPD/csv/'+str(sample)+'/demo.csv',header=0)
#             print(demo.shape)
#             print(demo.head())
            if X_df.empty:
...
#             print("y_df",y_df.shape)
    ...
    def save_output(self,labels,prob,logits):
        ...
        with open('./dataCOPD/output/'+'outputDict', 'wb') as fp:
    ...
    def save_outputImp(self,labels,prob,logits,importance,features):
        ...
        imp_df.to_csv('./dataCOPD/output/'+'feature_importance.csv', index=False)
