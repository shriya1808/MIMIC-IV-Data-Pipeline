...
def generate_patient_features_csv(cohort_output_name, root_dir="."):
    """
    Generates a CSV file with patient features (demographics + aggregated clinical codes)
    
    Args:
        cohort_output_name (str): The name of your cohort file inside ./dataHeart_Failure/cohort/ 
                                  (without .csv.gz extension). 
                                  Example: "cohort_non-icu_readmission_30_no_disease_filter"
    """
    
    # Define Paths
    cohort_path = f"{root_dir}/dataHeart_Failure/cohort/{cohort_output_name}.csv.gz"
    # cohort_path = f"./dataHeart_Failure/cohort/{cohort_output_name}.csv"
    diag_path = f"{root_dir}/dataHeart_Failure/features/preproc_diag.csv.gz"
    proc_path = f"{root_dir}/dataHeart_Failure/features/preproc_proc.csv.gz"
    med_path = f"{root_dir}/dataHeart_Failure/features/preproc_med.csv.gz"
    lab_path = f"{root_dir}/dataHeart_Failure/features/preproc_labs.csv.gz"
    
    output_filename = f"{root_dir}/patient_features_Heart_Failure.csv"
...
if __name__ == "__main__":
    # REPLACE THIS with the actual name of your cohort file found in ./dataHeart_Failure/cohort/
