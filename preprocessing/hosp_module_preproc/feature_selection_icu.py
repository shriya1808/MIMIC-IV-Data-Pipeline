...
if not os.path.exists("./dataHeart_Failure/features"):
    os.makedirs("./dataHeart_Failure/features")
if not os.path.exists("./dataHeart_Failure/features/chartevents"):
...
def feature_icu(cohort_output, version_path, diag_flag=True,out_flag=True,chart_flag=True,proc_flag=True,med_flag=True):
    if diag_flag:
        ...
        diag[['subject_id', 'hadm_id', 'stay_id', 'icd_code','root_icd10_convert','root']].to_csv("./dataHeart_Failure/features/preproc_diag_icu.csv.gz", compression='gzip', index=False)
        print("[SUCCESSFULLY SAVED DIAGNOSIS DATA]")
    
    if out_flag:  
        print("[EXTRACTING OUPTPUT EVENTS DATA]")
        out = preproc_out("./"+version_path+"/icu/outputevents.csv.gz", './dataHeart_Failure/cohort/'+cohort_output+'.csv.gz', 'charttime', dtypes=None, usecols=None)
        out[['subject_id', 'hadm_id', 'stay_id', 'itemid', 'charttime', 'intime', 'event_time_from_admit']].to_csv("./dataHeart_Failure/features/preproc_out_icu.csv.gz", compression='gzip', index=False)
        print("[SUCCESSFULLY SAVED OUPTPUT EVENTS DATA]")
    
    if chart_flag:
        print("[EXTRACTING CHART EVENTS DATA]")
        chart=preproc_chart("./"+version_path+"/icu/chartevents.csv.gz", './dataHeart_Failure/cohort/'+cohort_output+'.csv.gz', 'charttime', dtypes=None, usecols=['stay_id','charttime','itemid','valuenum','valueuom'])
        chart = drop_wrong_uom(chart, 0.95)
        chart[['stay_id', 'itemid','event_time_from_admit','valuenum']].to_csv("./dataHeart_Failure/features/preproc_chart_icu.csv.gz", compression='gzip', index=False)
        print("[SUCCESSFULLY SAVED CHART EVENTS DATA]")
    
    if proc_flag:
        print("[EXTRACTING PROCEDURES DATA]")
        proc = preproc_proc("./"+version_path+"/icu/procedureevents.csv.gz", './dataHeart_Failure/cohort/'+cohort_output+'.csv.gz', 'starttime', dtypes=None, usecols=['stay_id','starttime','itemid'])
        proc[['subject_id', 'hadm_id', 'stay_id', 'itemid', 'starttime', 'intime', 'event_time_from_admit']].to_csv("./dataHeart_Failure/features/preproc_proc_icu.csv.gz", compression='gzip', index=False)
        print("[SUCCESSFULLY SAVED PROCEDURES DATA]")
    
    if med_flag:
        print("[EXTRACTING MEDICATIONS DATA]")
        med = preproc_meds("./"+version_path+"/icu/inputevents.csv.gz", './dataHeart_Failure/cohort/'+cohort_output+'.csv.gz')
        med[['subject_id', 'hadm_id', 'stay_id', 'itemid' ,'starttime','endtime', 'start_hours_from_admit', 'stop_hours_from_admit','rate','amount','orderid']].to_csv('./dataHeart_Failure/features/preproc_med_icu.csv.gz', compression='gzip', index=False)
...
def preprocess_features_icu(cohort_output, diag_flag, group_diag,chart_flag,clean_chart,impute_outlier_chart,thresh,left_thresh):
    if diag_flag:
        ...
        diag = pd.read_csv("./dataHeart_Failure/features/preproc_diag_icu.csv.gz", compression='gzip',header=0)
        ...
        diag.to_csv("./dataHeart_Failure/features/preproc_diag_icu.csv.gz", compression='gzip', index=False)
        print("[SUCCESSFULLY SAVED DIAGNOSIS DATA]")
        
    if chart_flag:
        if clean_chart:   
            print("[PROCESSING CHART EVENTS DATA]")
            chart = pd.read_csv("./dataHeart_Failure/features/preproc_chart_icu.csv.gz", compression='gzip',header=0)
...
#                     print(f"{idx} not found")
            ...
            chart.to_csv("./dataHeart_Failure/features/preproc_chart_icu.csv.gz", compression='gzip', index=False)
            print("[SUCCESSFULLY SAVED CHART EVENTS DATA]")
...
def generate_summary_icu(diag_flag,proc_flag,med_flag,out_flag,chart_flag):
    ...
    if diag_flag:
        ...
        summary.to_csv('./dataHeart_Failure/summary/diag_summary.csv',index=False)
        summary['new_icd_code'].to_csv('./dataHeart_Failure/summary/diag_features.csv',index=False)


    if med_flag:
        med = pd.read_csv("./dataHeart_Failure/features/preproc_med_icu.csv.gz", compression='gzip',header=0)
        ...
        summary=summary.fillna(0)
        summary.to_csv('./dataHeart_Failure/summary/med_summary.csv',index=False)
        summary['itemid'].to_csv('./dataHeart_Failure/summary/med_features.csv',index=False)

    
    
    if proc_flag:
        proc = pd.read_csv("./dataHeart_Failure/features/preproc_proc_icu.csv.gz", compression='gzip',header=0)
        ...
        summary=summary.fillna(0)
        summary.to_csv('./dataHeart_Failure/summary/proc_summary.csv',index=False)
        summary['itemid'].to_csv('./dataHeart_Failure/summary/proc_features.csv',index=False)

        
    if out_flag:
        out = pd.read_csv("./dataHeart_Failure/features/preproc_out_icu.csv.gz", compression='gzip',header=0)
        ...
        summary=pd.merge(freq,total,on='itemid',how='right')
        summary=summary.fillna(0)
        summary.to_csv('./dataHeart_Failure/summary/out_summary.csv',index=False)
        summary['itemid'].to_csv('./dataHeart_Failure/summary/out_features.csv',index=False)
...
#         final.groupby('itemid')['missing%'].mean().reset_index()
        summary=summary.fillna(0)
        summary.to_csv('./dataHeart_Failure/summary/chart_summary.csv',index=False)
        summary['itemid'].to_csv('./dataHeart_Failure/summary/chart_features.csv',index=False)
...
def features_selection_icu(cohort_output, diag_flag,proc_flag,med_flag,out_flag,chart_flag,group_diag,group_med,group_proc,group_out,group_chart):
    if diag_flag:
        if group_diag:
            ...
            diag = pd.read_csv("./dataHeart_Failure/features/preproc_diag_icu.csv.gz", compression='gzip',header=0)
            features=pd.read_csv("./dataHeart_Failure/summary/diag_features.csv",header=0)
    ...
    if med_flag:       
        if group_med:   
            ...
            med = pd.read_csv("./dataHeart_Failure/features/preproc_med_icu.csv.gz", compression='gzip',header=0)
            features=pd.read_csv("./dataHeart_Failure/summary/med_features.csv",header=0)
            med=med[med['itemid'].isin(features['itemid'].unique())]
            print("Total number of rows",med.shape[0])
            med.to_csv('./dataHeart_Failure/features/preproc_med_icu.csv.gz', compression='gzip', index=False)
            print("[SUCCESSFULLY SAVED MEDICATIONS DATA]")
    ...
    if proc_flag:
        if group_proc:
            ...
            proc = pd.read_csv("./dataHeart_Failure/features/preproc_proc_icu.csv.gz", compression='gzip',header=0)
            features=pd.read_csv("./dataHeart_Failure/summary/proc_features.csv",header=0)
            proc=proc[proc['itemid'].isin(features['itemid'].unique())]
            print("Total number of rows",proc.shape[0])
            proc.to_csv("./dataHeart_Failure/features/preproc_proc_icu.csv.gz", compression='gzip', index=False)
            print("[SUCCESSFULLY SAVED PROCEDURES DATA]")
    ...
    if out_flag:
        if group_out:            
            ...
            out = pd.read_csv("./dataHeart_Failure/features/preproc_out_icu.csv.gz", compression='gzip',header=0)
            features=pd.read_csv("./dataHeart_Failure/summary/out_features.csv",header=0)
            out=out[out['itemid'].isin(features['itemid'].unique())]
            print("Total number of rows",out.shape[0])
            out.to_csv("./dataHeart_Failure/features/preproc_out_icu.csv.gz", compression='gzip', index=False)
    ...
    if chart_flag:
        if group_chart:            
            ...
            chart=pd.read_csv("./dataHeart_Failure/features/preproc_chart_icu.csv.gz", compression='gzip',header=0, index_col=None)
            
            features=pd.read_csv("./dataHeart_Failure/summary/chart_features.csv",header=0)
