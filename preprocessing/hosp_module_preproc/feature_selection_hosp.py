...
if not os.path.exists("./dataCOPD/summary"):
    os.makedirs("./dataCOPD/summary")
...
def feature_nonicu(cohort_output,version_path, diag_flag=True,lab_flag=True,proc_flag=True,med_flag=True):
    ...
    if diag_flag:
        ...
        diag = preproc_icd_module("./mimiciv/3.0/hosp/diagnoses_icd.csv.gz", './dataCOPD/cohort/'+cohort_output+'.csv.gz', './utils/mappings/ICD9_to_ICD10_mapping.txt', map_code_colname='diagnosis_code')
        diag[['subject_id', 'hadm_id', 'icd_code','root_icd10_convert','root']].to_csv("./dataCOPD/features/preproc_diag.csv.gz", compression='gzip', index=False)
        print("[SUCCESSFULLY SAVED DIAGNOSIS DATA]")

#     if lab_flag:    
#         out = preproc_out("./mimic-iv-1.0/icu/outputevents.csv.gz", './dataCOPD/cohort/'+cohort_output+'.csv.gz', 'charttime', dtypes=None, usecols=None)
#         out[['subject_id', 'hadm_id', 'stay_id', 'itemid', 'charttime', 'intime', 'event_time_from_admit']].to_csv("./dataCOPD/features/preproc_out_icu.csv.gz", compression='gzip', index=False)
    ...
    if proc_flag:
        ...
        proc = preproc_proc("./"+version_path+"/hosp/procedures_icd.csv.gz",'./dataCOPD/cohort/'+cohort_output+'.csv.gz', 'chartdate', 'base_anchor_year', dtypes=None, usecols=None)
        proc[['subject_id', 'hadm_id', 'icd_code','icd_version', 'chartdate', 'admittime', 'proc_time_from_admit']].to_csv("./dataCOPD/features/preproc_proc.csv.gz", compression='gzip', index=False)
        print("[SUCCESSFULLY SAVED PROCEDURES DATA]")
    
    if med_flag:
        print("[EXTRACTING MEDICATIONS DATA]")
        med = preproc_meds("./"+version_path+"/hosp/prescriptions.csv.gz", './dataCOPD/cohort/'+cohort_output+'.csv.gz','./utils/mappings/ndc_product.txt')
        med[['subject_id', 'hadm_id', 'starttime','stoptime','drug','nonproprietaryname', 'start_hours_from_admit', 'stop_hours_from_admit','dose_val_rx']].to_csv('./dataCOPD/features/preproc_med.csv.gz', compression='gzip', index=False)
        print("[SUCCESSFULLY SAVED MEDICATIONS DATA]")

    if lab_flag:
        print("[EXTRACTING LABS DATA]")
        lab = preproc_labs("./"+version_path+"/hosp/labevents.csv.gz", version_path,'./dataCOPD/cohort/'+cohort_output+'.csv.gz','charttime', 'base_anchor_year', dtypes=None, usecols=None)
        lab = drop_wrong_uom(lab, 0.95)
        lab[['subject_id', 'hadm_id', 'charttime', 'itemid','admittime','lab_time_from_admit','valuenum']].to_csv('./dataCOPD/features/preproc_labs.csv.gz', compression='gzip', index=False)
...
def preprocess_features_hosp(cohort_output, diag_flag,proc_flag,med_flag,lab_flag,group_diag,group_med,group_proc,clean_labs,impute_labs,thresh,left_thresh):
    ...
    if diag_flag:
        ...
        diag = pd.read_csv("./dataCOPD/features/preproc_diag.csv.gz", compression='gzip',header=0)
        ...
        diag.to_csv("./dataCOPD/features/preproc_diag.csv.gz", compression='gzip', index=False)
        print("[SUCCESSFULLY SAVED DIAGNOSIS DATA]")
    
    if med_flag:
        print("[PROCESSING MEDICATIONS DATA]")
        if group_med:           
            med = pd.read_csv("./dataCOPD/features/preproc_med.csv.gz", compression='gzip',header=0)
            if group_med:
            ...
            med.to_csv('./dataCOPD/features/preproc_med.csv.gz', compression='gzip', index=False)
            print("[SUCCESSFULLY SAVED MEDICATIONS DATA]")
    
    
    if proc_flag:
        print("[PROCESSING PROCEDURES DATA]")
        proc = pd.read_csv("./dataCOPD/features/preproc_proc.csv.gz", compression='gzip',header=0)
        if(group_proc=='ICD-9 and ICD-10'):
            ...
            proc.dropna().to_csv("./dataCOPD/features/preproc_proc.csv.gz", compression='gzip', index=False)
            ...
            print("Total number of rows",proc.shape[0])
            proc.to_csv("./dataCOPD/features/preproc_proc.csv.gz", compression='gzip', index=False)
        print("[SUCCESSFULLY SAVED PROCEDURES DATA]")
    ...
    if lab_flag:
        ...
        if clean_labs:   
            ...
            labs = pd.read_csv("./dataCOPD/features/preproc_labs.csv.gz", compression='gzip',header=0)
...
#             del labs['valueuom']
            labs.to_csv("./dataCOPD/features/preproc_labs.csv.gz", compression='gzip', index=False)
            print("[SUCCESSFULLY SAVED LABS DATA]")
...
def generate_summary_hosp(diag_flag,proc_flag,med_flag,lab_flag):
    ...
    if med_flag:
        ...
        summary['drug_name'].to_csv('./dataCOPD/summary/med_features.csv',index=False)
    ...
    if proc_flag:
        ...
        summary['icd_code'].to_csv('./dataCOPD/summary/proc_features.csv',index=False)
    ...
    if lab_flag:
        chunksize = 10000000
        labs=pd.DataFrame()
        for chunk in tqdm(pd.read_csv("./dataCOPD/features/preproc_labs.csv.gz", compression='gzip',header=0, index_col=None,chunksize=chunksize)):
            if labs.empty:
                labs=chunk
            else:
        ...
        summary['missing%']=100*(summary['missing_count']/summary['total_count'])
        summary=summary.fillna(0)
        summary.to_csv('./dataCOPD/summary/labs_summary.csv',index=False)
        summary['itemid'].to_csv('./dataCOPD/summary/labs_features.csv',index=False)
        ...
        if clean_labs:            
            ...
            for chunk in tqdm(pd.read_csv("./dataCOPD/features/preproc_labs.csv.gz", compression='gzip',header=0, index_col=None,chunksize=chunksize)):
                if labs.empty:
                    ...
                    # labs=labs.append(chunk, ignore_index=True)
                    labs=pd.concat([labs,chunk], ignore_index=True)
            features=pd.read_csv("./dataCOPD/summary/labs_features.csv",header=0)
