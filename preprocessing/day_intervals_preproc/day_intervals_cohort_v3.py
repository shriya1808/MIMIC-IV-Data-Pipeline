...
if not os.path.exists("./dataCKD/cohort"):
    os.makedirs("./dataCKD/cohort")
...
def extract_data(use_ICU:str, label:str, time:int, icd_code:str, root_dir, disease_label, cohort_output=None, summary_output=None):
    ...
    cohort[cols].to_csv(root_dir+"/dataCKD/cohort/"+cohort_output+".csv.gz", index=False, compression='gzip')
    ...
    with open(f"./dataCKD/cohort/{summary_output}.txt", "w") as f:
        f.write(summary)
